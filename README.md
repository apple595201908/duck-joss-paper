...
`flaring` 是獨立場景，不會立即跳出失敗結果。進入該場景後，正式成績停止、輸入停用，依序播放 `ignition`、`burst`、`inferno`、`smoke` 四階段發爐效果與完整音效，再轉入 `fail`。
