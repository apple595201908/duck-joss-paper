...
- 金紙量視覺化：遊戲正下方貼近底緣顯示加寬的紅金金紙台，圖案與文字保留足夠內距，並與鴨鴨保持間隔；紙堆會隨剩餘百分比從厚厚一疊逐步變薄。
- 以畫面判斷：正式遊戲不顯示底部熱度或速度進度條，玩家需觀察火焰、色彩與「發爐警告」。
- 金紙入爐演出：每次金紙會從鴨鴨手中沿拋物線飛行，接近爐口時縮小、向下旋入並被桶口遮住，搭配金色餘光強化投入感。
- 高手與新手有明顯秒數差：一般節奏約 20 秒，保守玩法約 30–40 秒，高手可在約 14–17 秒完成。
- 完整發爐演出：到達危險上限時，金爐會持續冒出往上高竄的大量火焰，接著湧出濃煙，再顯示失敗結果；不採用爆炸式衝擊波。
- 原創台式廟埕 BGM：以程式即時合成嗩吶感主旋律、堂鼓、鑼、鈸與梆子節奏，無外部歌曲、錄音或取樣。
- 宮廟數位計時器：緊貼畫面頂緣並避開鴨鴨頭毛，以淡金牌匾、朱紅描邊、對稱宮廟回紋角飾包住黑色顯示窗，使用程式繪製的鮮紅七段 LED 數字；「秒」以鮮紅字置於獨立深色金框中。
...
危險區效率獎勵會在熱度 55% 之後開始增加，86% 時達到最大值 `1.35`。紅色危急區從 78% 開始，只占最後 22%；獎勵集中在這段狹窄區間，讓熟練玩家能以更高發爐風險換取真正明顯的速度優勢。
...
| `riskPaperMaxBonus` | `1.35` | 高風險區的最大效率獎勵 |
| `riskPaperBonusStartRatio` | `0.55` | 風險獎勵開始點 |
| `riskPaperBonusFullRatio` | `0.86` | 完整風險獎勵點 |
...
| `criticalRatio` | `0.78` | 危急視覺開始點，紅區寬度為最後 22% |
| `tapThrowAnimationFrames` | `14` | 每次點擊延續投紙姿勢的 frame 數 |
| `FLARE_REACTION_FRAMES` | `132` | 發爐反應演出長度，60 Hz 下約 2.2 秒 |
...
`flaring` 是獨立場景，不會立即跳出失敗結果。進入該場景後，正式成績停止、輸入停用，依序播放 `ignition`、`surge`、`inferno`、`smoke` 四階段發爐效果與持續燃燒音效，再轉入 `fail`。火焰以金爐口為中心垂直竄升，不使用爆炸光圈或放射衝擊波。
...
- 快速連點能延續連貫投紙姿勢。
- 發爐反應一定先於失敗視窗。
- 發爐演出至少維持 120 frame，並完整經過四個視覺階段。
- 同一點擊完成與發爐時，完成優先。
- 暫停不會偷跑計時。
- 無腦狂點會在一半進度前發爐。
- 一般、保守與高手節奏落在指定時間區間。
...
`tests/audio.test.ts` 另外驗證 BGM 會產生有限數值、左右聲道差異、足夠音量與安全峰值，並拒絕不合用的取樣率。
...
全部聲音都由 Web Audio API 在玩家裝置上即時合成，不需額外下載音檔：
...
BGM 使用原創五聲音階旋律與 150 BPM 的活潑節奏，音色設計參考台灣北管／廟會常見的嗩吶、堂鼓、大鑼、小鑼、鈸與梆子組合。主旋律、低音、打擊樂、空間回音及所有音色均由 `src/game/audio.ts` 的數學波形與確定性雜訊合成，沒有下載、翻奏、重混或取樣任何第三方音樂錄音。

文化聲響研究參考：

- [文化部文化資產局：北管音樂戲曲](https://twh.boch.gov.tw/non_material/intro.aspx?id=744)
- [彰化縣文化局：北管音樂戲曲介紹](https://www.bocach.gov.tw/News_Content.aspx?n=4887&s=44598)

### 授權策略

開發時亦查核 [Creative Commons CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/) 的使用範圍。CC0 雖允許商業使用與改作，仍必須逐一確認素材確實由權利人正確標示。為把第三方授權與來源誤標風險降到最低，本專案最後沒有收錄任何網路 BGM 或聲音素材，而是採用完全原創的程式合成方案；音訊實作隨本專案程式碼依 MIT License 提供。
