...
- Keep the dedicated paper-supply card centered against the bottom edge during play, with visible clearance from the duck and separated from the left-side flare warning. Preserve generous horizontal padding between its gold frame, paper pile, title, and percentage. Its pile and percentage must derive from the same remaining-paper ratio.
- Keep the count-up timer flush to the top edge and clear of the duck's crest, in an ample pale-gold Taiwanese temple-plaque casing with vermilion trim and a dark display window; avoid purple casing colors. Draw the large red time using complete seven-segment LED glyphs, never decorative Chinese-character seals, and keep every segment inside the frame. Render the `秒` label in bright red inside its own high-contrast dark badge.
